...
type SortKey = "name" | "area" | "subArea" | "totalAmount" | "totalKg" | "orderCount";
...
const Customers = () => {
  ...
  const subAreaOptions = useMemo(() => {
    // @ts-ignore
    const subs = new Set(orders?.map((o: any) => o.sub_area).filter(Boolean));
    return Array.from(subs).sort() as string[];
  }, [orders]);

  const deleteCustomer = useMutation({ mutationFn: async (id: string) => { const { error } = await supabase.from("customers").delete().eq("id", id); if (error) throw error; }, onSuccess: () => { toast.success("Customer deleted"); queryClient.invalidateQueries({ queryKey: ["customers"] }); }, onError: (e: any) => toast.error(e.message) });
  ...
  const filtered = useMemo(() => {
    ...
    list.sort((a: any, b: any) => {
      const statsA = customerStats?.[a.id]; const statsB = customerStats?.[b.id];
      let cmp = 0;
      switch (sortKey) {
        case "name": cmp = (a.name || "").localeCompare(b.name || ""); break;
        case "area": cmp = (a.areas?.area_name || "").localeCompare(b.areas?.area_name || ""); break;
        case "subArea": cmp = (statsA?.latestSubArea || "").localeCompare(statsB?.latestSubArea || ""); break;
    ...
    <div>
      ...
      <div className="flex flex-col gap-4 mb-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-3 bg-card p-4 rounded-xl border shadow-sm">
            <Input value={filterName} onChange={(e) => setFilterName(e.target.value)} placeholder="Name..." className="h-9" />
            <Input value={filterPhone} onChange={(e) => setFilterPhone(e.target.value)} placeholder="Phone..." className="h-9" />
            <Select value={filterArea} onValueChange={setFilterArea}><SelectTrigger className="h-9"><SelectValue placeholder="Area" /></SelectTrigger><SelectContent><SelectItem value="all">All Areas</SelectItem>{areas?.map((a: any) => <SelectItem key={a.id} value={a.id}>{a.area_name}</SelectItem>)}</SelectContent></Select>
            <div className="relative"><Input list="custSubAreaOpts" value={filterSubArea} onChange={(e) => setFilterSubArea(e.target.value)} placeholder="Sub Area..." className="h-9" /><datalist id="custSubAreaOpts">{subAreaOptions.map(opt => <option key={opt} value={opt} />)}</datalist></div>
            <Input value={filterAddress} onChange={(e) => setFilterAddress(e.target.value)} placeholder="Address..." className="h-9" />
        </div>
      </div>
